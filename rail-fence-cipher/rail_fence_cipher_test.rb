...
class RailFenceCipherTest < Minitest::Test
  ...
  def test_encode_with_one_rail
    assert_equal 'One rail, only one rail', RailFenceCipher.encode('One rail, only one rail', 1)
  end
  ...
  def test_encode_with_less_letters_than_rails
    assert_equal 'More rails than letters', RailFenceCipher.encode('More rails than letters', 24)
  end
  ...
  def test_decode_with_one_rail
    assert_equal 'ABCDEFGHIJKLMNOP', RailFenceCipher.decode('ABCDEFGHIJKLMNOP', 1)
  end

  def test_decode_with_two_rails
    assert_equal 'XOXOXOXOXOXOXOXOXO', RailFenceCipher.decode('XXXXXXXXXOOOOOOOOO', 2)
  end
